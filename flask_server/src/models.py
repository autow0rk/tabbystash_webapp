...
class TabGroup(db.Model):
    ...
    timeTabGroupCreated = db.Column(db.DateTime, nullable=False, server_default=utcnow())#default=datetime.now(tz=timezone('US/Eastern')))#default=utcnow)#default=datetime.strftime(datetime.now(tz=timezone('US/Eastern')), "%Y-%m-%d %H:%M:%S"))
...
class UserTabGroups(db.Model): # stores composite keys linking Users to TabGroups(eg. user:1 tabGroup:3, user:1 tabGroup: 50, etc.)
    __tablename__ = 'userTabGroup'

    #id = db.Column(db.Integer, primary_key=True)
    tabGroupID = db.Column(db.Integer, db.ForeignKey('tabGroup.id'), primary_key=True, nullable=False)
