...
export default function Dashboard(props) {
  function dashboardOutput(){
    if(props.callForTabDataFailed){ // the callForTabData key is only attached to the props object when calling the Flask API for a users tab data failed; it's never attached in any other situation, so there's no need to check the value. If it even exists on the props object, then we know that the call to the Flask API failed.
...
export async function getServerSideProps({ req }) {
  ...
  if(loggedIn) {
    ...
    tabDataFromServer = await axios.get("http://localhost:5000/auth/getUserTabData", {
      headers: {
        Cookie: req.headers.cookie,
      },
    })
    .then((res) => {
      ...
      if(res.data.success) {
        ...
        console.log('the res.data.success to give to tabDataFromServer: ', res.data.success);
    ...
    if(callForTabDataSucceeded){ // we want to know if calling the Flask API for getting tab data worked, because we need to tell the difference between: (1) no tab data showing because calling our Flask API failed (2) no tab data showing because the user has no tab data on record. If making the call for the tab data was successful but the user has no data, an empty object will be returned to the <DisplayTabData/> component, which will render an empty dashboard. If making the call for tab data wasn't successful, then an error messaage will be displayed in the dashboard page
      return {
        props: {
    ...
    return {
      props: { // if calling the Flask API for the user data failed, the result of callForTabDataSucceeded will be false. The value of callForTabDataFailed is the negated value of that, so it will be true. This is done for easier understanding when reading the value into the props object in the dashboard component.
